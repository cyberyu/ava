...
flags.DEFINE_integer(
    "class_no", 5,
    "specify how many class the model is trained or")
...
class ColaProcessor(DataProcessor):
  ...
  def get_labels(self):
    """See base class."""
    #return ["0", "1"]
    a = [str(x) for x in range(FLAGS.class_no)]
    return a
...
def main(_):
  ...
  if FLAGS.do_predict:
    ...
    temp_result = estimator.evaluate(input_fn=predict_input_fn, steps=eval_steps)
    output_predict_file = os.path.join(FLAGS.output_dir, "predict_results.txt")
    with tf.gfile.GFile(output_predict_file, "w") as writer:
      tf.logging.info("***** Eval results *****")
      for key in sorted(temp_result.keys()):
        tf.logging.info("  %s = %s", key, str(temp_result[key]))
        writer.write("%s = %s\n" % (key, str(temp_result[key])))    
